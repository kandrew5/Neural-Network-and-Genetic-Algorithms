...
class GeneticAlgorithm(object):
    ...
    def run(self):
        ...
        for i in range(1, self.generations):
            best_by_gen.append(self.best_individual()[0])
            holdout.append(self.best_individual()[1])
            if i>2:
                if (best_by_gen[i-1] == best_by_gen[i-2]):
                    stop_best += 1
                    if  stop_best == 25:
                        print("Stopped at " + str(i) + " generation by best")
                        break
                else:
                    stop_best = 0
            mean = 0
            ...
            mean_fitness.append(mean/self.population_size)
            if i > 2:
                if ((mean_fitness[i - 1] - mean_fitness[i - 2]) / mean_fitness[i - 1]) < 0.01:
                    stop_mean += 1
                    if stop_mean == 40:
                        print("Stopped at " + str(i) + " generation by mean")
                        break
                else:
                    stop_mean = 0
            self.create_next_generation()
        return best_by_gen, holdout
